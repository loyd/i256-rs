...
/// Division of numerator by a u64 divisor
#[inline]
pub fn div_rem_small<const N: usize>(numerator: &[u64; N], divisor: u64) -> ([u64; N], u64) {
    let mut rem = 0u64;
...
impl<T, const N: usize> Deref for ArrayPlusOne<T, N> {
    type Target = [T];

    #[inline(always)]
    fn deref(&self) -> &Self::Target {
        let x = self as *const Self;
        // SAFETY: Safe  since the undelying struct is POD with N+! elements.
        unsafe { slice::from_raw_parts(x as *const T, N + 1) }
    }
}

impl<T, const N: usize> DerefMut for ArrayPlusOne<T, N> {
    #[inline(always)]
    fn deref_mut(&mut self) -> &mut Self::Target {
        let x = self as *mut Self;
        // SAFETY: Safe  since the undelying struct is POD with N+! elements.
        unsafe { slice::from_raw_parts_mut(x as *mut T, N + 1) }
    }
}
