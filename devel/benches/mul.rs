...
#[inline]
fn mul_cryptobi(x: crypto_bigint::U256, y: crypto_bigint::U256) -> crypto_bigint::U256 {
    x.wrapping_mul(&y)
}
    ...
    ($name:ident, $gen:ident, $prefix:literal) => {
        fn $name(criterion: &mut Criterion) {
            ...
            let bnum_data: Vec<[U256; 2]> = u128_data
                ...
                .collect();
            let crypto_bigint_data: Vec<[crypto_bigint::U256; 2]> = u128_data
                .iter()
                .map(|x| [input::cryptobi_from_u128(x[0], x[1]), input::cryptobi_from_u128(x[2], x[3])])
                .collect();
            ...
            op_generator!(@4 group, concat!($prefix, "::u256-i256"), wrapping_mul_u128, u128_data.iter());
            op_generator!(@2 group, concat!($prefix, "::u256-bnum"), mul_bnum, bnum_data.iter());
            op_generator!(@2 group, concat!($prefix, "::u256-crypto-bigint"), mul_cryptobi, crypto_bigint_data.iter());

            let half_data: Vec<(u128, u128, u64)> = u128_data.iter().map(|x| (x[0], x[1], u64::gen(RandomGen::$gen, &mut rng)))
                .collect();

            op_generator!(@3 group, concat!($prefix, "::u256-i256-small"), wrapping_mul_small_u128, u128_data.iter());
            op_generator!(@3tup group, concat!($prefix, "::u256-i256-half"), wrapping_mul_half_u128, half_data.iter());
        }
    }
}
