    ...
    ($name:ident, $strategy:expr, $prefix:literal) => {
        fn $name(criterion: &mut Criterion) {
            ...
            add_bench!(
                ...
                bnum_udata.iter(),
                bench_op!(wrapping_sub, BnumU256)
            );
            add_bench!(
                group,
                concat!($prefix, "::unsigned-crypto"),
                crypto_udata.iter(),
                bench_op!(@ref wrapping_sub, CryptoU256)
            );
            add_bench!(
                group,
                concat!($prefix, "::unsigned-256"),
                u256_udata.iter(),
                bench_op!(wrapping_sub, i256::u256)
            );
            add_bench!(
                group,
                concat!($prefix, "::unsigned-256-ulimb"),
                ulimb_udata.iter(),
                bench_op!(wrapping_sub_ulimb, i256::u256, i256::ULimb)
            );
            ...
            add_bench!(
                ...
                bnum_idata.iter(),
                bench_op!(wrapping_sub, BnumI256)
            );
            add_bench!(
                group,
                concat!($prefix, "::signed-256"),
                i256_idata.iter(),
                bench_op!(wrapping_sub, i256::i256)
            );
            add_bench!(
                group,
                concat!($prefix, "::signed-256-ulimb"),
                ulimb_idata.iter(),
                bench_op!(wrapping_sub_ulimb, i256::i256, i256::ULimb)
            );
            add_bench!(
                group,
                concat!($prefix, "::signed-256-ilimb"),
                ilimb_idata.iter(),
                bench_op!(wrapping_sub_ilimb, i256::i256, i256::ILimb)
            );
        }
    };
}
